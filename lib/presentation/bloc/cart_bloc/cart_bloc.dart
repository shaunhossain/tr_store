...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  CartBloc(this.repository) : super(const CartState()) {
    on<CartEvent>((event, emit) async {
      await event.map(
          productList: (_ProductList req) async {
            emit(state.copyWith(status: CartStatus.loading));
            final result = await repository.getAllCartProducts();
            var listOfProductEntity = result
                .map((element) => ProductResponse(
                    id: element.id,
                    slug: element.slug,
                    title: element.title,
            ...
            emit(state.copyWith(
                ...
                listOfProducts: listOfProductEntity));
          },
          ...
          updateCart: (_UpdateCart req) async {
            emit(const CartState());
            await repository.updateProduct(
                productResponse: req.productResponse);
            emit(
              state.copyWith(
                status: CartStatus.success,
              ),
          ...
          deleteCart: (_DeleteCart req) async {
            emit(const CartState());
